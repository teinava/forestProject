...
public:
    ...
    gameObject(int imageWidth, int imageHeight)
    {
        ...
        x = (imageWidth * 2.0f) / windowWidth;
        y = (imageHeight * 2.0f) / windowHeight;
...
void gameplay(int currentFrame, float slimeDensity)
{
    ...
    if (dog->getHealth() <= 0)
    {
        ...
        float gameOverScale = 0.09;
        score->drawText(-0.3, 0.14, gameOverScale, gameOverScale, "GAME OVER");
        gameOver = true;
    }
}
